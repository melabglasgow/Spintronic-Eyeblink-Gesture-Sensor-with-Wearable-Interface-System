...
/* This sketch implement part of Nordic custom LED blinkR Service (LBS).
...
const uint8_t LBS_UUID_CHR_blinkR[] =
{
  0x23, 0xD1, 0xBC, 0xEA, 0x5F, 0x78, 0x23, 0x15,
...
BLECharacteristic lsbblinkR(LBS_UUID_CHR_blinkR);
BLECharacteristic lsbLED(LBS_UUID_CHR_LED);

// Use on-board blinkR if available, else use A0 pin
//#ifdef PIN_blinkR1
 
  uint8_t blinkR = A1;   //left blink
//#endif

//#ifdef PIN_BUTTON1      //right blink

  uint8_t button = A0;
//#endif

// CPB blinkR active state is HIGH, all other is low
#ifdef ARDUINO_NRF52840_CIRCUITPLAY
  #define blinkR_ACTIVE     HIGH
#else
  #define blinkR_ACTIVE     LOW
#endif
...
void setup()
{
  pinMode(blinkR, blinkR_ACTIVE ? INPUT_PULLDOWN : INPUT_PULLUP);
  blinkRState = (blinkR_ACTIVE == digitalRead(blinkR));
  ...
  // Setup the LED-blinkR service using
  Serial.println("Configuring the LED-blinkR Service");
  ...
  // Configure blinkR characteristic
  // Properties = Read + Notify
  // Permission = Open to read, cannot write
  // Fixed Len  = 1 (blinkR state)
  lsbblinkR.setProperties(CHR_PROPS_READ | CHR_PROPS_NOTIFY);
  lsbblinkR.setPermission(SECMODE_OPEN, SECMODE_NO_ACCESS);
  lsbblinkR.setFixedLen(1);
  lsbblinkR.begin();
  lsbblinkR.write8(blinkRState);
...
void loop()
{
  delay(10); // poll blinkR every 10 ms

  uint8_t newState = (blinkR_ACTIVE == digitalRead(blinkR));
  uint8_t texas = (BUTTON_ACTIVE == digitalRead(button));



 if ( texas != buttonState)
  {
    buttonState = texas;
    lsbButton.write8(buttonState);
  ...
  // only notify if blinkR state changes
  if ( newState != blinkRState)
  {
    blinkRState = newState;
    lsbblinkR.write8(blinkRState);

    // notify all connected clients
    for (uint16_t conn_hdl=0; conn_hdl < MAX_PRPH_CONNECTION; conn_hdl++)
    {
      if ( Bluefruit.connected(conn_hdl) && lsbblinkR.notifyEnabled(conn_hdl) )
      {
        lsbblinkR.notify8(conn_hdl, blinkRState);
      }
    }
  }
}
